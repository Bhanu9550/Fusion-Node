...
const SignUp = () => {
    ...
    let [shimmerUi, setShimmerUi] = useState(false)
    ...
    async function submitUser(e) {
        ...
        try {
            ...
            if(err.request.status === 0){
                setUserError("Server is Down or Network Issue , Try again after some time");
            }
            else if(err.response.data.response){
                setUserError(err.response.data.error)
            }
            else if(err.response.data.error.errors.email){
                setUserError(err.response.data.error.errors.email.message)
            }
            else if(err.response.data.error.errors.password){
                setUserError(err.response.data.error.errors.password.message)
            }
            else{
                setUserError("Server Error, Please Try after some Time")
            }
        }
        finally{
            setShimmerUi(false)
    ...
    async function handleUsername(e) {
        if(e.target.value.length >=6){
            try {
                await api.post(`/userCheck`, { username: userDetails.username })    
                setUserError("")
            } 
            catch (err) {
                ...
                <div className="signup-right">
                    ...
                    <div className="signup-footer">
                        <span className="signup-footer-text">
                            ...
                            <Link className="signin-link" to="/SignIn" type="button">
                                Sign In
                            </Link>
                        </span>
                        <Link className="signup-back" to="/" type="button">
                            ← Go Back
                        </Link>
                    </div>
