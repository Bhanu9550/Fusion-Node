    ...
    try {
      ...
      if (bannerFile) {
        const bannerResult = await uploadToCloudinary(
          bannerFile.buffer,
          "fusion-node/projects/banners",
        );

        projectData.bannerImageUrl = bannerResult.secure_url;
        projectData.bannerImagePublicId = bannerResult.public_id;
      } else {
        projectData.bannerImageUrl = null;
      }

      if (logoFile) {
        const logoResult = await uploadToCloudinary(
          logoFile.buffer,
          "fusion-node/projects/logos",
        );

        projectData.logoUrl = logoResult.secure_url;
        projectData.logoPublicId = logoResult.public_id;
      } else {
        projectData.logoUrl = null;
      }

      //* Set owner details securely
      const owner = await User.findById(req.user.user_id).select(
    ...
    if (statuses.length) {
      ...
      statuses.forEach((s) => {
        if (s === "Recruiting") recruitmentStatuses.push("open");
          ...
          stages.push("💻 Development", "🧪 Testing", "🚀 Deployment");
      });
      const or = [];
      if (recruitmentStatuses.length)
        or.push({ recruitmentStatus: { $in: recruitmentStatuses } });
...
projectPageController.get("/projectData", verifyToken, async (req, res) => {
  ...
  const response = await getProjectData(
    { status, category, role, experience, search },
    req.user.user_id,
  );
  if (response instanceof Error) {
    return res
      .status(500)
      .json({ error: response.message || "Internal Server Error" });
  }
  ...
  try {
    ...
    const pendingApplications = ownedProjects.flatMap((p) =>
      p.applications
        ...
        .map((a) => ({
          ...a.toObject(),
          project: { _id: p._id, name: p.name, logoUrl: p.logoUrl },
        })),
    );
  ...
  async (req, res) => {
    try {
      ...
      if (project.recruitmentStatus !== "open") {
        return res.status(400).json({
          message: "This project is not accepting applications right now",
        });
      }
      if (String(project.userId) === String(userId)) {
        return res
          .status(400)
          .json({ message: "You can't apply to your own project" });
      }
      ...
      const alreadyConnected =
        owner.followers.some((id) => String(id) === String(userId)) ||
        owner.following.some((id) => String(id) === String(userId));
      const alreadyRequested = owner.followRequests.some(
      ...
      if (!alreadyConnected && !alreadyRequested) {
        if (!owner.isPrivate) {
          ...
          await User.findByIdAndUpdate(userId, {
            $addToSet: { following: project.userId },
          });
        } else {
          await User.findByIdAndUpdate(project.userId, {
  ...
  async (req, res) => {
    try {
      const project = await Project.findById(req.params.projectId).populate(
        "applications.userId",
        "fullname username profilePicture designation",
      );
      if (!project)
        return res.status(404).json({ message: "Project not found" });
      if (String(project.userId) !== String(req.user.user_id)) {
        return res
          .status(403)
          .json({ message: "Only the project owner can view applications" });
      }
  ...
  async (req, res) => {
    try {
      ...
      if (String(project.userId) !== String(req.user.user_id)) {
        return res
          .status(403)
          .json({ message: "Only the project owner can review applications" });
      }
...
projectPageController.post(
  ...
  async (req, res) => {
    try {
      ...
      const invitee =
        await User.findById(inviteeId).select("fullname username");
  ...
  async (req, res) => {
    try {
      ...
      if (!["Accepted", "Declined"].includes(decision)) {
        return res
          .status(400)
          .json({ message: "decision must be Accepted or Declined" });
      }
      ...
      if (String(invite.userId) !== String(req.user.user_id)) {
        return res
          .status(403)
          .json({ message: "This invite is not addressed to you" });
      }
  ...
  async (req, res) => {
    try {
      ...
      if (isSelf && !isOwner && project.leavingPolicy === "approval") {
        return res.status(400).json({
          message:
            "This project requires approval to leave. Please submit a leave request instead.",
      ...
      await notify({
        ...
        title: isSelf
          ? `A member left ${project.name}`
          : `You were removed from ${project.name}`,
        message: isSelf
          ? `${member.roleName} left the team.`
          : `You were removed from the ${member.roleName} role.`,
        project: project._id,
      });
  ...
  async (req, res) => {
    try {
      ...
      const member = project.teamMembers.find(
        (m) =>
          String(m.userId) === String(req.user.user_id) &&
          m.status === "approved",
      );
      if (!member)
        return res
          .status(404)
          .json({ message: "You're not an active member of this project" });
      ...
      const requester = await User.findById(req.user.user_id).select(
        "fullname username",
      );
      await notify({
        recipient: project.userId,
  ...
  async (req, res) => {
    try {
      ...
      if (String(project.userId) !== String(req.user.user_id)) {
        return res
          .status(403)
          .json({ message: "Only the project owner can view leave requests" });
      }
      const pending = project.teamMembers.filter(
        (m) => m.leaveRequestStatus === "pending",
      );
...
projectPageController.patch(
  "/:projectId/leave-requests/:memberId",
  verifyToken,
  async (req, res) => {
    try {
      const { projectId, memberId } = req.params;
      ...
      if (String(project.userId) !== String(req.user.user_id)) {
        return res.status(403).json({
          message: "Only the project owner can decide on leave requests",
        });
      }
      ...
      if (!member || member.leaveRequestStatus !== "pending") {
        return res
          .status(404)
          .json({ message: "No pending leave request found" });
      }
  ...
  async (req, res) => {
    try {
      ...
      if (String(project.userId) !== String(req.user.user_id)) {
        return res
          .status(403)
          .json({ message: "Only the project owner can edit this project" });
      }

      const body = req.body.projectDetails
      ...
      const oldLogoPublicId = project.logoPublicId;
      const oldBannerPublicId = project.bannerImagePublicId;
      ...
      if (bannerFile) {
        ...
        project.bannerImageUrl = bannerResult.secure_url;
        project.bannerImagePublicId = bannerResult.public_id;
      }
      ...
      if (logoFile) {
        ...
        project.logoUrl = logoResult.secure_url;
        project.logoPublicId = logoResult.public_id;
      }

      // --- ROLES LOGIC ---
      if (Array.isArray(body.roles)) {
        const incomingIds = body.roles
          .filter((r) => r._id)
          .map((r) => String(r._id));
      ...
      // Delete old Cloudinary images AFTER successful database save
      if (bannerFile && oldBannerPublicId) {
        try {
...
projectPageController.get("/:projectId", verifyToken, async (req, res) => {
  try {
    ...
    if (
      ...
      String(project.userId?._id) !== String(req.user.user_id)
    ) {
