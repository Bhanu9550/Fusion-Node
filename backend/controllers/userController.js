...
const PUBLIC_FIELDS =
  "-password -hashedPass -refreshToken -__v -email -phoneNumber -dateOfBirth";
...
userController.get("/me/edit", verifyToken, async (req, res) => {
  try {
    const user = await User.findById(req.user.user_id).select(
      "-password -hashedPass -refreshToken -__v",
    );
    res.json({ user });
  } catch (err) {
  ...
  async (req, res) => {
    try {
      ...
      if (updates.isPrivate !== undefined)
        updates.isPrivate =
          updates.isPrivate === "true" || updates.isPrivate === true;
      if (updates.emailNotification !== undefined)
        updates.emailNotification = Number(updates.emailNotification);
      if (updates.pushNotification !== undefined)
        updates.pushNotification = Number(updates.pushNotification);
      ...
      const currentUser = await User.findById(req.user.user_id).select(
        "profilePicture profilePicturePublicId coverPicture coverPicturePublicId",
      );
      if (!currentUser)
        return res.status(404).json({ message: "User not found" });

      const oldProfilePicPublicId = currentUser.profilePicturePublicId;
      const oldCoverPicPublicId = currentUser.coverPicturePublicId;
      ...
      if (profilePicFile) {
        const result = await uploadToCloudinary(
          ...
          "fusion-node/users/profile-pictures",
        );
      ...
      if (coverPicFile) {
        const result = await uploadToCloudinary(
          coverPicFile.buffer,
          "fusion-node/users/cover-pictures",
        );

        updates.coverPicture = result.secure_url;
        updates.coverPicturePublicId = result.public_id;
      }

      //* 4. Save to Database
      const user = await User.findByIdAndUpdate(req.user.user_id, updates, {
        returnDocument: "after",
      }).select("-password -hashedPass -refreshToken -__v");

      //* 5. Delete Old Files AFTER successful DB update
      if (profilePicFile && oldProfilePicPublicId) {
        try {
      ...
      if (coverPicFile && oldCoverPicPublicId) {
        try {
          await cloudinary.uploader.destroy(oldCoverPicPublicId);
        } catch (error) {
          console.error("Failed to delete old cover picture:", error);
        }
      }
...
userController.get("/:username", verifyToken, async (req, res) => {
  try {
    ...
    const isFollowedBy = profileUser.following.some(
      (id) => String(id) === String(viewerId),
    );
    const hasSentRequest = profileUser.followRequests.some(
    ...
    const [createdProjects, joinedProjects] = await Promise.all([
      ...
      Project.find({
        teamMembers: {
          $elemMatch: { userId: profileUser._id, status: "approved" },
        },
        status: { $ne: "deleted" },
      })
        .select("name logoUrl stage recruitmentStatus")
...
userController.post("/:userId/follow", verifyToken, async (req, res) => {
  try {
    ...
    await User.findByIdAndUpdate(targetId, {
      $push: { followRequests: { userId: viewerId } },
    });
    await notify({
      recipient: targetId,
      sender: viewerId,
...
userController.post(
  "/me/follow-requests/:requesterId/:decision",
  verifyToken,
  async (req, res) => {
    try {
      ...
      if (!["accept", "ignore"].includes(decision)) {
        return res
          .status(400)
          .json({ message: "decision must be accept or ignore" });
      }
      const viewerId = req.user.user_id;

      const me = await User.findById(viewerId);
      const hadRequest = me.followRequests.some(
        (r) => String(r.userId) === String(requesterId),
      );
      if (!hadRequest)
        return res
          .status(404)
  ...
  try {
    ...
    if (!isSelf) {
      const isFollowing = user.followers.some(
        (u) => String(u._id) === String(viewerId),
      );
      ...
      if (!(isFollowing && isFollowedBy)) {
        return res.status(403).json({
          message: "You can only see this once you both follow each other",
        });
      }
    }
